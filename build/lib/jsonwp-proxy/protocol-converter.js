...
const DEFAULT_LOG = armor_support_1.logger.getLogger('Protocol Converter');
class ProtocolConverter {
    ...
    async proxySetWindow(url, method, body) {
        const bodyObj = armor_support_1.util.safeJsonParse(body);
    ...
    async proxySetValue(url, method, body) {
        const bodyObj = armor_support_1.util.safeJsonParse(body);
        if (lodash_1.default.isPlainObject(bodyObj) && (armor_support_1.util.hasValue(bodyObj.text) || armor_support_1.util.hasValue(bodyObj.value))) {
            let { text, value } = bodyObj;
            if (armor_support_1.util.hasValue(text) && !armor_support_1.util.hasValue(value)) {
            ...
            else if (!armor_support_1.util.hasValue(text) && armor_support_1.util.hasValue(value)) {
