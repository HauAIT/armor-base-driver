...
const DEFAULT_LOG = armor_support_1.logger.getLogger('WD Proxy');
...
class JWProxy {
    ...
    async proxy(url, method, body = null) {
        ...
        if (armor_support_1.util.hasValue(body) && method !== 'GET') {
        ...
        try {
            ...
            if (armor_support_1.util.hasValue(e.response)) {
                if (!isResponseLogged) {
                    const error = truncateBody(e.response.data);
                    this.log.info(armor_support_1.util.hasValue(e.response.status)
