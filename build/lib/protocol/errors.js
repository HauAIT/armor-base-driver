...
const mjsonwpLog = armor_support_1.logger.getLogger('MJSONWP');
const w3cLog = armor_support_1.logger.getLogger('W3C');
...
class ProxyRequestError extends es6_error_1.default {
    constructor(err, responseError, httpStatus) {
        let responseErrorObj = armor_support_1.util.safeJsonParse(responseError);
    ...
    getActualError() {
        ...
        if (armor_support_1.util.hasValue(this.jsonwp?.status) && armor_support_1.util.hasValue(this.jsonwp?.value)) {
            return errorFromMJSONWPStatusCode(this.jsonwp.status, this.jsonwp.value);
        }
        else if (armor_support_1.util.hasValue(this.w3c) && lodash_1.default.isNumber(this.w3cStatus) && this.w3cStatus >= 300) {
...
function getResponseForW3CError(err) {
    ...
    if (!isProtocolError(err)) {
        err = armor_support_1.util.hasValue(err.status)
