...
/**
 *
 * @param {import('armor-types').ExternalDriver} driver
 * @param {string?} [sessionId]
 * @returns {import('armor-types').ArmorLogger}
 */
function getLogger(driver, sessionId = null) {
    ...
    let logPrefix = dstDriver.constructor
        ? `${dstDriver.constructor.name}@${armor_support_1.node.getObjectId(dstDriver).substring(0, 8)}`
        : 'ArmorDriver';
    if (sessionId) {
        logPrefix += ` (${sessionId.substring(0, 8)})`;
    }
    return armor_support_1.logger.getLogger(logPrefix);
}
...
/**
 *
 * @param {import('armor-types').Core} driver
...
function routeConfiguringFunction(driver) {
    ...
    // passed in as defined by Armor plugins, so we need to add those to the default list
    return function addRoutes(app, { basePath = constants_1.DEFAULT_BASE_PATH, extraMethodMap = {} } = {}) {
        ...
        for (const [path, methods] of lodash_1.default.toPairs(allMethods)) {
            for (const [method, spec] of lodash_1.default.toPairs(methods)) {
                ...
                buildHandler(app, method, `${basePath}${path}`, spec, driver, isSessionCommand(/** @type {import('armor-types').DriverMethodDef} */ (spec).command));
            }
        }
    };
}
        ...
        try {
            ...
            if (spec.deprecated && !deprecatedCommandsLogged.has(spec.command)) {
                ...
                getLogger(driver, req.params.sessionId).warn(`Command '${spec.command}' has been deprecated and will be removed in a future ` +
                    `version of Armor or your driver/plugin. Please use a different method or contact the ` +
            ...
            // only want to do these when we have a session command; the Armor driver must be
            ...
            if (armor_support_1.util.hasValue(driverRes)) {
                if (armor_support_1.util.hasValue(driverRes.status) &&
