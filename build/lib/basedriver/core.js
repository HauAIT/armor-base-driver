...
class DriverCore {
    constructor(opts = {}, shouldValidateCaps = true) {
        this._log = armor_support_1.logger.getLogger(helpers_1.default.generateDriverLogPrefix(this));
        // setup state
        this.opts = opts;
        // use a custom tmp dir to avoid losing data and app when computer is
        // restarted
        this.opts.tmpDir = this.opts.tmpDir || process.env.ARMOR_TMP_DIR || node_os_1.default.tmpdir();
    ...
    assertFeatureEnabled(name) {
        if (!this.isFeatureEnabled(name)) {
            throw new Error(`Potentially insecure feature '${name}' has not been ` +
                ...
                `the documented instructions at http://armor.io/docs/en/2.0/guides/security/`);
        }
    }
