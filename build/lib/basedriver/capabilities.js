...
exports.PREFIXED_ARMOR_OPTS_CAP = exports.promoteArmorOptionsForObject = exports.promoteArmorOptions = exports.stripArmorPrefixes = exports.isStandardCap = exports.findNonPrefixedCaps = exports.ARMOR_VENDOR_PREFIX = exports.mergeCaps = exports.validateCaps = exports.processCapabilities = exports.parseCaps = exports.STANDARD_CAPS = void 0;
...
const ARMOR_VENDOR_PREFIX = 'armor:';
exports.ARMOR_VENDOR_PREFIX = ARMOR_VENDOR_PREFIX;
const PREFIXED_ARMOR_OPTS_CAP = `${ARMOR_VENDOR_PREFIX}options`;
exports.PREFIXED_ARMOR_OPTS_CAP = PREFIXED_ARMOR_OPTS_CAP;
/**
...
function validateCaps(caps, constraints = /** @type {C} */ ({}), opts = {}) {
    ...
    const validationErrors = desired_caps_1.validator.validate(lodash_1.default.pickBy(caps, armor_support_1.util.hasValue), constraints, {
        fullMessages: false,
    });
    if (validationErrors) {
        let message = [];
...
/** @type {StringKeyOf<import('armor-types').StandardCapabilities>[]} */ ([
    'browserName',
    'browserVersion',
    'platformName',
    'acceptInsecureCerts',
 ...
 * @returns {Capabilities<C>}
 */
function stripArmorPrefixes(caps) {
    // split into prefixed and non-prefixed.
    // non-prefixed should be standard caps at this point
    const [prefixedCaps, nonPrefixedCaps] = lodash_1.default.partition(lodash_1.default.keys(caps), (cap) => String(cap).startsWith(ARMOR_VENDOR_PREFIX));
    ...
    // Strip out the 'armor:' prefix
    for (let prefixedCap of prefixedCaps) {
        const strippedCapName = /** @type {StringKeyOf<Capabilities<C>>} */ (prefixedCap.substring(ARMOR_VENDOR_PREFIX.length));
    ...
    // If we found standard caps that were incorrectly prefixed, throw an exception (e.g.: don't accept 'armor:platformName', only accept just 'platformName')
    if (badPrefixedCaps.length > 0) {
        logger_1.default.warn(`The capabilities ${JSON.stringify(badPrefixedCaps)} are standard capabilities and do not require "armor:" prefix`);
    }
    return strippedCaps;
}
exports.stripArmorPrefixes = stripArmorPrefixes;
/**
...
function parseCaps(caps, constraints = /** @type {C} */ ({}), shouldValidateCaps = true) {
    ...
    // Strip out the 'armor:' prefix from all
    let strippedRequiredCaps = stripArmorPrefixes(requiredCaps);
    /** @type {Capabilities<C>[]} */
    let strippedAllFirstMatchCaps = allFirstMatchCaps.map(stripArmorPrefixes);
...
function processCapabilities(w3cCaps, constraints = /** @type {C} */ ({}), shouldValidateCaps = true) {
    ...
    if (!armor_support_1.util.hasValue(matchedCaps)) {
 ...
 */
function promoteArmorOptionsForObject(obj) {
    const armorOptions = obj[PREFIXED_ARMOR_OPTS_CAP];
    if (!armorOptions) {
        return obj;
    }
    if (!lodash_1.default.isPlainObject(armorOptions)) {
        throw new errors_1.errors.SessionNotCreatedError(`The ${PREFIXED_ARMOR_OPTS_CAP} capability must be an object`);
    }
    if (lodash_1.default.isEmpty(armorOptions)) {
        return obj;
    }
    logger_1.default.debug(`Found ${PREFIXED_ARMOR_OPTS_CAP} capability present; will promote items inside to caps`);
    /**
     * @param {string} capName
     */
    const shouldAddVendorPrefix = (capName) => !capName.startsWith(ARMOR_VENDOR_PREFIX);
    const verifyIfAcceptable = (/** @type {string} */ capName) => {
        if (!lodash_1.default.isString(capName)) {
            throw new errors_1.errors.SessionNotCreatedError(`Capability names in ${PREFIXED_ARMOR_OPTS_CAP} must be strings. '${capName}' is unexpected`);
        }
        if (isStandardCap(capName)) {
            throw new errors_1.errors.SessionNotCreatedError(`${PREFIXED_ARMOR_OPTS_CAP} must only contain vendor-specific capabilties. '${capName}' is unexpected`);
        }
        return capName;
    };
    const preprocessedOptions = (0, lodash_1.default)(armorOptions)
        .mapKeys((value, /** @type {string} */ key) => verifyIfAcceptable(key))
        .mapKeys((value, key) => (shouldAddVendorPrefix(key) ? `${ARMOR_VENDOR_PREFIX}${key}` : key))
        .value();
    ...
    if (overwrittenKeys.length > 0) {
        logger_1.default.warn(`Found capabilities inside ${PREFIXED_ARMOR_OPTS_CAP} that will overwrite ` +
            `capabilities at the top level: ${JSON.stringify(overwrittenKeys)}`);
    }
    return lodash_1.default.cloneDeep({
        ... /** @type {NSCapabilities<C>} */(lodash_1.default.omit(obj, PREFIXED_ARMOR_OPTS_CAP)),
        ...preprocessedOptions,
    });
}
exports.promoteArmorOptionsForObject = promoteArmorOptionsForObject;
/**
...
exports.promoteArmorOptions = promoteArmorOptions;
/**
 * @typedef {import('armor-types').Constraints} Constraints
 * @typedef {import('armor-types').Constraint} Constraint
 * @typedef {import('armor-types').StringRecord} StringRecord
 * @typedef {import('armor-types').BaseDriverCapConstraints} BaseDriverCapConstraints
 */
/**
 * @template {Constraints} C
 * @typedef {import('armor-types').ConstraintsToCaps<C>} ConstraintsToCaps
 */
/**
...
/**
 * @template {Constraints} C
 * @typedef {import('armor-types').NSCapabilities<C>} NSCapabilities
 */
/**
 * @template {Constraints} C
 * @typedef {import('armor-types').Capabilities<C>} Capabilities
 */
/**
 * @template {Constraints} C
 * @typedef {import('armor-types').W3CCapabilities<C>} W3CCapabilities
 */
/**
 * @template T
