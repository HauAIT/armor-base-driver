 ...
 * @param {Partial<import('armor-types').ServerArgs>} [cliArgs]
...
async function createServer(app, cliArgs) {
    ...
    const zipped = lodash_1.default.zip(await bluebird_1.default.all(certKey.map((p) => armor_support_1.fs.exists(p))), ['certificate', 'key'], certKey);
...
async function server(opts) {
    ...
    return await new bluebird_1.default(async (resolve, reject) => {
        ...
        try {
            const armorServer = configureHttp({
                httpServer,
                reject,
                keepAliveTimeout,
            });
            ...
            for (const updater of serverUpdaters) {
                await updater(app, armorServer, cliArgs);
            }
...
function configureServer({ app, addRoutes, allowCors = true, basePath = constants_1.DEFAULT_BASE_PATH, extraMethodMap = {}, }) {
    ...
    // make sure armor never fails because of a file size upload limit
...
/**
 * Monkeypatches the `http.Server` instance and returns a {@linkcode ArmorServer}.
...
function configureHttp({ httpServer, reject, keepAliveTimeout }) {
     ...
     * @type {ArmorServer}
     */
    const armorServer = /** @type {any} */ (httpServer);
    armorServer.addWebSocketHandler = websocket_1.addWebSocketHandler;
    armorServer.removeWebSocketHandler = websocket_1.removeWebSocketHandler;
    armorServer.removeAllWebSocketHandlers = websocket_1.removeAllWebSocketHandlers;
    armorServer.getWebSocketHandlers = websocket_1.getWebSocketHandlers;
    // http.Server.close() only stops new connections, but we need to wait until
    // all connections are closed and the `close` event is emitted
    const close = armorServer.close.bind(armorServer);
    armorServer.close = async () => await new bluebird_1.default((resolve, reject) => {
    ...
    armorServer.on('connection', 
    /** @param {ArmorServerSocket} socket */ (socket) => {
    ...
    armorServer.on('request', function (req, res) {
        const socket = /** @type {ArmorServerSocket} */ (req.connection || req.socket);
...
/**
 * Starts an {@linkcode ArmorServer}
 ...
 */
/**
 * @typedef {import('armor-types').ArmorServer} ArmorServer
 * @typedef {import('armor-types').ArmorServerSocket} ArmorServerSocket
 */
/**
 * @typedef {import('armor-types').MethodMap<import('armor-types').ExternalDriver>} MethodMap
 */
/**
 ...
 * @property {import('armor-types').ServerArgs} [cliArgs]
 * @property {string} [hostname]
 * @property {boolean} [allowCors]
 * @property {string} [basePath]
 * @property {MethodMap} [extraMethodMap]
 * @property {import('armor-types').UpdateServerCallback[]} [serverUpdaters]
