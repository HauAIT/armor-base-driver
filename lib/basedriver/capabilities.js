...
function stripArmorPrefixes(caps) {
  ...
  const [prefixedCaps, nonPrefixedCaps] = _.partition(_.keys(caps), (cap) =>
    String(cap).startsWith(ARMOR_VENDOR_PREFIX)
  );
  ...
  for (let prefixedCap of prefixedCaps) {
    const strippedCapName = /** @type {StringKeyOf<Capabilities<C>>} */ (
      prefixedCap.substring(ARMOR_VENDOR_PREFIX.length)
    );
  ...
  if (badPrefixedCaps.length > 0) {
    log.warn(
      `The capabilities ${JSON.stringify(
        badPrefixedCaps
      )} are standard capabilities and do not require "armor:" prefix`
    );
  }
  return strippedCaps;
}
...
function promoteArmorOptionsForObject(obj) {
  ...
  if (!_.isPlainObject(armorOptions)) {
    throw new errors.SessionNotCreatedError(
      `The ${PREFIXED_ARMOR_OPTS_CAP} capability must be an object`
    );
  }
  if (_.isEmpty(armorOptions)) {
    return obj;
  }

  log.debug(
    `Found ${PREFIXED_ARMOR_OPTS_CAP} capability present; will promote items inside to caps`
  );
  ...
  const verifyIfAcceptable = (/** @type {string} */ capName) => {
    if (!_.isString(capName)) {
      throw new errors.SessionNotCreatedError(
        `Capability names in ${PREFIXED_ARMOR_OPTS_CAP} must be strings. '${capName}' is unexpected`
      );
    }
    if (isStandardCap(capName)) {
      throw new errors.SessionNotCreatedError(
        `${PREFIXED_ARMOR_OPTS_CAP} must only contain vendor-specific capabilties. '${capName}' is unexpected`
      );
    }
    return capName;
  };
  const preprocessedOptions = _(armorOptions)
  ...
  if (overwrittenKeys.length > 0) {
    log.warn(
      `Found capabilities inside ${PREFIXED_ARMOR_OPTS_CAP} that will overwrite ` +
        `capabilities at the top level: ${JSON.stringify(overwrittenKeys)}`
    );
  }
  return _.cloneDeep({
    .../** @type {NSCapabilities<C>} */ (_.omit(obj, PREFIXED_ARMOR_OPTS_CAP)),
    ...preprocessedOptions,
  });
}
